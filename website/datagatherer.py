...
#finaldata = {'stages':[]}
finaldata = json.loads(open("data.json",'r').read()[13:-1])

stages = schedule["data"]["stages"]

for i in [0,1,3,4]:
    stage = stages[i]["matches"]
    weeks = stages[i]["weeks"]
    ...
    for j in range(0,len(stage)):
        ...
        if j<70:
            if stagedata['regular'][j]['completed']:
                '''matchdata = stagedata['regular'][j]
                time = match["startDateTS"]
                for w in range(5):
                    if weeks[w]["startDate"]<time and time<weeks[w]["endDate"]: matchdata['week']=w
...
open('data.json','w').write("stageRecord='"+json.dumps(finaldata)+"'")
print("Data Updated")
