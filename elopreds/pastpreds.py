...
pteams = ["VAN","NYE","TOR","PHI","ATL","SFS","SEO","BOS"]
#pteams = ["SFS","LDN","VAN","GLA","NYE","DAL","HZS","SHD"]
...
def simulatePlayoffs(pteams):
    def simulateMatch(t1,t2,firstto):
        if any([(t1 in c and t2 in c) for c in completed]):
            return next(c for c in completed if (t1 in c and t2 in c))[2]
...
for t in sorted(pteams,key=lambda x:playoffplacements[x][0],reverse=True):
...
positions = {t:0 for t in teams}

for x in range(loops):
    results = simulateStage3()

    for t in teams:
        pl = results.index(t)+1
        if pl<=8: playoffs[t]+=1
        positions[t]+=pl

table = [["Team","Playoffs","Position"]]
for t in sorted(teams,key=lambda x:positions[x],reverse=False):
    table.append([t]+["{:.2%}".format(playoffs[t]/loops)]+[positions[t]/loops])
